...
class Member(Base):
    ...
    member_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, server_default="gen_random_uuid()")
    ...
    # Relationships
    identities: Mapped[List["MemberIdentity"]] = relationship(back_populates="member", cascade="all, delete-orphan")
    ...
    component_memberships: Mapped[List["ComponentMember"]] = relationship(back_populates="member")
...
class MemberIdentity(Base):
    ...
    member_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("catalog.members.member_id", ondelete="CASCADE"), primary_key=True
    )
...
class ComponentMember(Base):
    ...
    component_id: Mapped[str] = mapped_column(Text, primary_key=True)
    member_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("catalog.members.member_id"), primary_key=True
    )
    ...
    # Relationships
    component: Mapped["Component"] = relationship(back_populates="members", foreign_keys=[system, component_id])
...
class Message(Base):
    ...
    author_member_id: Mapped[Optional[UUID]] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("catalog.members.member_id")
    )
    ...
    # Relationships
    author_member: Mapped[Optional["Member"]] = relationship(back_populates="messages", foreign_keys=[author_member_id])
    component: Mapped["Component"] = relationship(back_populates="messages", foreign_keys=[system, component_id])
...
class MessageMention(Base):
    ...
    message_id: Mapped[str] = mapped_column(
        Text, ForeignKey("silver.messages.message_id", ondelete="CASCADE"), primary_key=True
    )
    ...
    member_id: Mapped[Optional[UUID]] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("catalog.members.member_id"))
    display_text: Mapped[Optional[str]] = mapped_column(Text)

    # Relationships
    message: Mapped["Message"] = relationship(back_populates="mentions")
    mentioned_member: Mapped[Optional["Member"]] = relationship(foreign_keys=[member_id])
...
class Reaction(Base):
    ...
    message_id: Mapped[str] = mapped_column(
        Text, ForeignKey("silver.messages.message_id", ondelete="CASCADE"), primary_key=True, index=True
    )
    reaction: Mapped[str] = mapped_column(Text, primary_key=True)
    member_id: Mapped[Optional[UUID]] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("catalog.members.member_id"), primary_key=True, index=True
    )
    created_at_ts: Mapped[datetime] = mapped_column(TIMESTAMPTZ, nullable=False, server_default="now()")
