...
from casting.apps.cast.discord.utils.helpers import (
    format_response,
    format_code_block,
    format_git_log,
    format_git_status,
...
class GitCommands(commands.Cog):
    ...
    async def git_commit(self, ctx, *, message: str):
        ...
        async with ctx.typing():
            response = await self.bot.api_client.git_commit(
                message=message, author_name=str(ctx.author), author_email=f"{ctx.author.id}@discord.user"
            )
    ...
    async def git_log(self, ctx, limit: int = 10):
        ...
        embed = discord.Embed(title="📜 Git Log", description=log_msg, color=discord.Color.blue())
    ...
    async def _handle_merge_conflicts(self, ctx, branch_name: str):
        ...
        conflict_embed.add_field(
            name="🔄 Auto-Commit Conflicts", value="Stage and commit all conflicts as-is", inline=False
        )
        conflict_embed.add_field(
            name="🔧 Resolve Conflicts", value="Show conflict details and choose resolution", inline=False
        )
        conflict_embed.add_field(name="❌ Cancel Merge", value="Abort the merge operation", inline=False)
        ...
        async def auto_commit_callback(interaction):
            if interaction.user != ctx.author:
                await interaction.response.send_message(
                    "❌ Only the command user can make this choice.", ephemeral=True
                )
                return
            ...
            async with ctx.typing():
                ...
                if add_response.get("success", True):
                    commit_response = await self.bot.api_client.git_commit(
                        f"Merge {branch_name} with conflicts", str(ctx.author), f"{ctx.author.id}@discord.user"
                    )
                    if commit_response.get("success", True):
                        await interaction.followup.send("✅ **Conflicts staged and committed successfully**")
                    else:
                        await interaction.followup.send(
                            f"❌ **Commit failed:** {commit_response.get('message', 'Unknown error')}"
                        )
                else:
                    await interaction.followup.send(
                        f"❌ **Staging failed:** {add_response.get('message', 'Unknown error')}"
                    )
            view.stop()
    ...
    async def _show_conflict_resolution(self, ctx, interaction):
        ...
        if not conflicts_response.get("success", True):
            await interaction.followup.send(
                f"❌ **Failed to get conflict details:** {conflicts_response.get('error', 'Unknown error')}"
            )
            return
        ...
        if diff_response.get("success", True):
            ...
            conflict_embed.add_field(name="📄 Conflict Content", value=f"```diff\n{truncated_diff}\n```", inline=False)
        ...
        async def accept_all_callback(btn_interaction):
            if btn_interaction.user != ctx.author:
                await btn_interaction.response.send_message(
                    "❌ Only the command user can resolve conflicts.", ephemeral=True
                )
                return
            ...
            async with ctx.typing():
                ...
                if add_response.get("success", True):
                    ...
                    await btn_interaction.followup.send(
                        f"❌ **Failed to stage:** {add_response.get('message', 'Unknown error')}"
                    )
            view.stop()
    ...
    async def git_list_branches(self, ctx):
        ...
        embed = discord.Embed(title="🌳 Git Branches", description=branch_msg, color=discord.Color.green())
        await ctx.send(embed=embed)
